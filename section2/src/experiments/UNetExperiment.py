...
class UNetExperiment:
    ...
    def train(self):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            target = batch['seg']
            target = target.to(self.device, torch.long)
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for i, batch in enumerate(self.val_loader):
                # validation code that will compute loss on a validation sample
                data   = batch['image'].to(self.device, torch.float)
                target = batch['seg'].to(self.device, torch.long)
                # do forward pass
                pred_val = self.model(data)
                prediction_softmax = F.softmax(pred_val, dim=1)
                # compute loss function
                val_loss = self.loss_function(pred_val, target[:, 0, :, :])
                print(f"Batch {i}. Data shape {data.shape} Loss {val_loss}")
    ...
    def run_test(self):
        ...
        for i, x in enumerate(self.test_data):
            ...
            #  Complete the implementation, you can look up definition of Jaccard index 
            # on Wikipedia. If you completed it correctly 
            # (and if you picked your train/val/test split right ;)),
            ...
            # STAND-OUT SUGGESTION: By way, consider also outputting:
