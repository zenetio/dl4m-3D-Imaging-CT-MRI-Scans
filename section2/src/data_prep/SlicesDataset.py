...
class SlicesDataset(Dataset):
    ...
    def __init__(self, data):
        ...
        for i, d in enumerate(data):
            for j in range(d["image"].shape[2]):    # changed from 0 t0 2
    ...
    def __getitem__(self, idx):
        ...
        # The tensor needs to be of shape [1, patch_size, patch_size]
        # Don't forget that you need to put a Torch Tensor into your dictionary element's value
        # Hint: the 3D data sits in self.data variable, the id of the 3D volume from data array
        ...
        # lets take the image and the label slices to work with
        image = self.data[slc[0]]["image"][:,:,slc[1]]
        label = self.data[slc[0]]["seg"][:,:,slc[1]]

        # Lets make sure we have one-hot encoding in our label
        # The visualization shows we have two label classes and the readme file point as 
        # anterior and posterior segments of the hippocampus. We can make a conversion and train the segment
